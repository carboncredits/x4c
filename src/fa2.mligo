...
let retire_tokens (param : retire) (storage : storage) : result =
    List.map (
        fun (p: retire_tokens): operation ->
            Tezos.emit "%retire" (Bytes.pack {
                retiring_party = p.retiring_party ;
                token_id = p.token_id ;
                amount = p.amount ;
                retiring_data = p.retiring_data ;
            })
    )
    param
    ,
...
let add_token_id (param : token_metadata list) (storage : storage) : result =
    ...
    (fun (s, d : storage * token_metadata) ->
        { s with token_metadata =
            ...
            | (None, m) -> m
            | (Some _, _m) -> (failwith error_ID_ALREADY_IN_USE : (token_id, token_metadata) big_map) } )
    storage
    param
