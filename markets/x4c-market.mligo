...
let token_is_approved (token : token) (storage : storage) : bool = 
    match (Tezos.call_view "get_proxy" "storage" storage.x4c_oracle : address option) with 
...
let post_for_sale (token, data : token_for_sale * sale_data) (storage : storage) : result = 
    ...
    if not token_is_approved { token_address = token.token_address ; token_id = token.token_id ; } storage 
