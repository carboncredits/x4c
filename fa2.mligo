...
let transfer (param : transfer list) (storage : storage) : result = 
    ...
    (fun (storage, p : storage * transfer) : storage -> 
        ...
        (fun (storage, p : storage * transfer_to) : storage -> 
            let (to, token_id, qty, operator) = (p.to_, p.token_id, p.amount, Tezos.sender) in 
            let owner = from in 
            // check permissions 
            if Tezos.sender <> from && not is_operator { token_owner = owner ; token_operator = operator ; token_id = token_id ; } qty storage.operators 
            ...
            let operators = 
                if Tezos.sender <> from // thus this is an operator acting
                then update_balance 
...
let mint_tokens (param : mint) (storage : storage) : result =
    // check permissions 
    if Tezos.source <> storage.oracle then (failwith error_PERMISSIONS_DENIED : result) else 
    // mint tokens 
    ([] : operation list),
    List.fold 
...
let retire_tokens (param : retire) (storage : storage) : result =
    ...
    (fun (s, p : (storage * retire_tokens)) : storage -> 
        // check permissions 
        if Tezos.sender <> p.retiring_party && not (is_operator { token_owner = p.retiring_party ; token_operator = Tezos.sender ; token_id = p.token_id ; } p.amount s.operators) 
        then (failwith error_PERMISSIONS_DENIED : storage) else
        // update storage 
        { storage with 
...
let add_token_id (param : token_metadata list) (storage : storage) : result = 
    if Tezos.sender <> storage.oracle then (failwith error_PERMISSIONS_DENIED : result) else
    ([] : operation list),
    List.fold_left
